...
export function FixedScreen() {
  ...
  return (
    <main className={styles.fixed}>
      ...
      {/* Табак не упоминаем: в РИСсити его не продают, и лишнее исключение
          только удлиняет строку, которую официант читает через плечо гостя.
          В правилах акции (`legal-content.ts`) формулировка своя — она
          юридическая и меняется отдельным решением. */}
      <p className={styles.menuFinePrint}>
        Скидка действует на весь счёт, кроме алкогольной продукции. Не выдаётся деньгами.
      </p>
